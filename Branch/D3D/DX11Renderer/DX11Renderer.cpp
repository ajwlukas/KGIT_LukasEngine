    ...
    if (dc != nullptr)
    {
        ...
        swapChain->ResizeBuffers(1, width, height, DXGI_FORMAT_R8G8B8A8_UNORM, 0);
         ...
         CreateAndSetDepthStencilView();
          ...
          testTexture[0]->Release();
          testSRV[0]->Release();
          testRTV[0]->Release();
          testTexture[1]->Release();
          testSRV[1]->Release();
          testRTV[1]->Release();
         TestCreateRTT();
...
void DX11Renderer::BeginRender()
{
    ...
    dc->ClearRenderTargetView(testRTV[0], black);
    dc->ClearRenderTargetView(testRTV[1], black);
    dc->ClearRenderTargetView(testRTV[2], black);
...
void DX11Renderer::TestInit()
{
    ...
    main->material->SetShader(L"DefferedRenderPS.hlsl");
    main->material->SetDiffuseMap(&testSRV[2]);
    main->material->SetNormalMap(&testSRV[1]);
    //normalCanvas->material->SetShader(L"");
    //main = new Canvas(0.0f, 0.0f, 1.0f, 1.0f);
    //canvas->transform.pos.z -= 1.0f;
    canvas->SetSRV(&testSRV[0]);
    normalCanvas->SetSRV(&testSRV[1]);
    albedoCanvas->SetSRV(&testSRV[2]);
...
void DX11Renderer::TestRender()
{
    ...
    //오브젝트들
    dc->OMSetRenderTargets(3, &testRTV[0], depthStencilView);
...
{ /// test code
    ...
    device->CreateTexture2D(&desc, NULL, &testTexture[0]);
    device->CreateTexture2D(&desc, NULL, &testTexture[1]);
    device->CreateTexture2D(&desc, NULL, &testTexture[2]);
    ...
    device->CreateRenderTargetView(testTexture[0], &rtvDesc, &testRTV[0]);
    device->CreateRenderTargetView(testTexture[1], &rtvDesc, &testRTV[1]);
    device->CreateRenderTargetView(testTexture[2], &rtvDesc, &testRTV[2]);
    ...
    device->CreateShaderResourceView(testTexture[0], &srvDesc, &testSRV[0]);
    device->CreateShaderResourceView(testTexture[1], &srvDesc, &testSRV[1]);
    device->CreateShaderResourceView(testTexture[2], &srvDesc, &testSRV[2]);
}
